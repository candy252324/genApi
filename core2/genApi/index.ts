...
import { IInterface, IApiModel, IParsered, IParams } from '../types'
...
function genStation(data: IParsered) {
  ...
  writeApiToFile(fileGroup, { outputDir, apiBody: data.apiBody, httpTpl: data.httpTpl })
  writeInterfaceToFile(data.interfaces, { outputDir })
}
...
function writeApiToFile(
  ...
  apiList.forEach((item) => {
    ...
    fs.access(outputDir, (err) => {
      ...
      // 写入目标目录
      const targetFile = path.join(outputDir, `${fileName}.ts`)
...
/** interface 写入 */
function writeInterfaceToFile(interfaces: IInterface[], config: { outputDir: string }) {
