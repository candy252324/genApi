...
/** 通用登录 */
export function loginDoLogin  (data:any, config?: AxiosRequestConfig) :AxiosPromise<ResResultApiResponse>{
  return Http.post('/api/login/doLogin', data, config)
}

/** 退出登陆 */
export function loginDoLogout  (data:any, config?: AxiosRequestConfig) :AxiosPromise<ResResultApiResponse>{
  return Http.get('/api/login/doLogout', data, config)
}

/** 发送手机登录验证码 */
export function loginSendPhoneValidateCode  (data:any, config?: AxiosRequestConfig) :AxiosPromise<ResResultApiResponse>{
